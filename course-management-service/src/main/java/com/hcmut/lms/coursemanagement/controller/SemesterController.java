...
@RestController
@RequestMapping("${prefix-api}/semesters")
...
public class SemesterController {
  ...
  public ResponseEntity<?> getSemestersByAcademicYearId(
      @PathVariable UUID academicYearId, @RequestParam(required = false) Integer page,
      @RequestParam(required = false) Integer size) {
    if (page != null && size != null) {
      PageResponse<SemesterResponse> response = semesterService.getSemestersByAcademicYearId(
          academicYearId, page, size);
      return ResponseEntity.ok(response);
    }
  ...
  @GetMapping("/me/remain")
  public ResponseEntity<List<SemesterResponse>> getRemainSemester(@CurrentUser CurrentUserInfo currentUser) {
    return ResponseEntity.ok(semesterService.getRemainSemester(currentUser.getId()));
  }
