...
public class SpecializationServiceImpl implements SpecializationService {
  ...
  public SpecializationResponse updateSpecialization(UUID id, SpecializationRequest request) {
    ...
    if (request.getCode() != null && !request.getCode()
        .equals(specialization.getCode()) && specializationRepository.existsByCode(request.getCode())) {
      throw new EntityExistsException("Specialization with code " + request.getCode() + " already exists");
    }
  ...
  public PageResponse<SpecializationResponse> getSpecializationsByDepartmentId(UUID departmentId, int page, int size) {
    log.info(
        "Getting specializations by department id: {} with pagination - page: {}, size: {}", departmentId, page, size);
  ...
  @Override
  public List<SpecializationResponse> getListSpecializationOptionByMe(UUID studentId) {
    log.info("Getting specialization options for student id: {}", studentId);
    UserResponse user = userServiceClient.getUserById(studentId);

    return specializationRepository.findByDepartmentId(user.getDepartmentId())
        .stream()
        .map(specializationMapper::toResponse)
        .toList();
  }
