...
public class OnetFlattenedRequirementAssembler {
    ...
    public List<OnetFlattenedRequirement> fromDwa(List<TasksToDwa> mappings, Map<String, DwaReference> dwaById) {
        Map<String, BigDecimal> maxImportanceByDwa = mappings.stream().collect(Collectors.groupingBy(TasksToDwa::getDwaId,
                Collectors.collectingAndThen(
                        Collectors.mapping(m -> m.getTaskStatement() != null ? m.getTaskStatement().getTaskType() : null,
                                Collectors.toList()),
                        this::maxImportanceFromTaskTypes)));
